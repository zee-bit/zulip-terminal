...
class PopUpView(urwid.Frame):
    def __init__(
        ...
        if header:
            header = urwid.Pile(urwid.SimpleFocusListWalker(header))
        if footer:
            footer = urwid.Pile(urwid.SimpleFocusListWalker(footer))
    ...
    def calculate_popup_height(
        ...
        """
        body_height = sum(widget.rows((popup_width,)) for widget in body)
        header_height = (
            sum(widget.rows((popup_width,)) for widget in header) if header else 0
        )
        footer_height = (
            sum(widget.rows((popup_width,)) for widget in footer) if footer else 0
        )
        return body_height + header_height + footer_height
...
class EmojiPickerView(PopUpView):
    ...
    def __init__(
        ...
        controller: Any,
        title: str,
        emoji_units: List[Tuple[str, str]],
        message: Message,
        view: Any,
    ) -> None:
        self.width = 40
